...
echo "=== KAWTHAR CART MATH FINAL FIX ==="

STAMP="$(date +%F-%H%M%S)"
mkdir -p backups js

tar -czf "backups/backup-before-cart-math-final-$STAMP.tar.gz" \
  index.html shop.html product.html checkout.html js css 2>/dev/null || true

cat > js/kawthar-cart-core.js <<'JS'
(function () {
  "use strict";

  const AUTHORITATIVE_KEY = "kawthar_cart";
  ...
  function clear() {
    ...
    try {
      window.dispatchEvent(new CustomEvent("kawthar:cart-updated", { detail: [] }));
    } catch {}
...
for name in pages:
    ...
    # Remove duplicate cart core links
    text = re.sub(r'\s*<script src="\./js/kawthar-cart-core\.js\?v=[^"]*"></script>', '', text)

    # Add fresh cart core in head before old page scripts
    if "</head>" in text:
        text = text.replace(
            "</head>",
            f'  <script src="./js/kawthar-cart-core.js?v=math-final-{ver}"></script>\n</head>',
            1
        )

    p.write_text(text, encoding="utf-8")

print("Cart core final math fix linked.")
print("Version:", ver)
PY
...
echo "=== DONE ==="
echo "Open checkout:"
echo "https://constantly-professionals-devoted-ground.trycloudflare.com/checkout.html?v=math-final-$STAMP"
