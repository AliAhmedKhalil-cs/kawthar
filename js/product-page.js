...
const applyStaticTranslations = () => {
  const lang = localStorage.getItem("kawtharabdo_lang") || "en";
...
const initLanguage = () => {
  ...
  document.querySelector("#langToggleBtn")?.addEventListener("click", () => {
    ...
    localStorage.setItem("kawtharabdo_lang", newLang);
    applyStaticTranslations();
    rerenderSidePanels();
    rerenderPage();
    initSearch();
...
const initStickyBar = () => {
  ...
  const origApply = window.__kawtharabdoLangObservers || [];
  origApply.push(updateBarContent);
  window.__kawtharabdoLangObservers = origApply;
};
